# B-0036 두 레이어 연산별 계측을 원자적으로 실행하고 엄격히 검증합니다.
...
if __package__:
    ...
    from tools import ablate_official_two_layer_attribution as attribution
    from tools import ablate_official_two_layer_closure as closure
else:
    import ablate_official_layer as common
    import ablate_official_two_layer_attribution as attribution
    import ablate_official_two_layer_closure as closure


CASES = closure.CASES
_FORMAT = "k3x-official-two-layer-operation-attribution-v1"
_BENCHMARK = "B-0036"
_SCOPE = "official-two-layer-operation-attribution"
_OPERATION_FIELDS = (
...
_RUNNER_FIELDS = (*attribution._RUNNER_FIELDS, *_OPERATION_FIELDS)
_RAW_FIELDS = _RUNNER_FIELDS
_CSV_FIELDS = ("name", "raw_json_sha256", *_RAW_FIELDS)
_SUMMARY_FIELDS = attribution._SUMMARY_FIELDS
...
def _validate_record(
    ...
    *,
    ...
    baseline = {
        field: record[field]
        for field in attribution._RAW_FIELDS
    }
    baseline["schema"] = attribution._FORMAT
    attribution._validate_record(
        baseline,
        name=name,
        mode=mode,
        warmups=warmups,
    ...
    if record.get("schema") != _FORMAT:
        raise RuntimeError(f"{name} operation attribution schema diverged")
    values = {field: record.get(field) for field in _OPERATION_FIELDS}
    if any(type(value) is not int or value < 0 for value in values.values()):
        raise RuntimeError(f"{name} operation timing diverged")
    if (
        record["front_device_nanoseconds"]
        != values["front_kda_device_nanoseconds"]
        + values["front_route_device_nanoseconds"]
        + values["front_unclassified_device_nanoseconds"]
        or record["tail_device_nanoseconds"]
        != values["tail_ffn_device_nanoseconds"]
        + values["tail_unclassified_device_nanoseconds"]
    ):
        raise RuntimeError(f"{name} operation formula diverged")
    if mode == "host-round-trip":
        if any(values.values()):
            raise RuntimeError(f"{name} operation formula diverged")
    elif (
        values["front_kda_device_nanoseconds"] <= 0
        or values["front_route_device_nanoseconds"] <= 0
        or values["tail_ffn_device_nanoseconds"] <= 0
    ):
        raise RuntimeError(f"{name} operation timing diverged")
...
def _run_case(
    ...
    *,
    ...
    command = [
        ...
        "--operation-attribution", "true",
    ]
    result = subprocess.run(command, capture_output=True, text=True)
    if result.returncode:
        raise RuntimeError(
            result.stderr.strip() or "official operation attribution failed"
        )
    return common._parse_json(result.stdout, f"{mode} operation output")
...
def verify_summary(
    ...
    *,
    ...
    if manifest is not None and closure.manifest_identity(Path(manifest)) != identity:
        raise RuntimeError("summary manifest identity diverged")
    if any(
        not closure._hex(summary.get(field))
        for field in (
            "artifact_sha256",
            "manifest_sha256",
            "oracle_sha256",
