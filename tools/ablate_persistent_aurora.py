# B-0018 replay와 persistent AURORA의 정확성 및 draft traffic을 쌍대로 측정합니다.
...
PAIRS = (
    ("fixed-2-token", "replay-fixed-2-token", "persistent-fixed-2-token"),
    ("adaptive-token", "replay-adaptive-token", "persistent-adaptive-token"),
    ("fixed-2-expert", "replay-fixed-2-expert", "persistent-fixed-2-expert"),
    (
        "adaptive-expert",
        "replay-adaptive-expert",
        "persistent-adaptive-expert",
...
def _diagnostic(
    ...
    runner: Path,
    output: Path,
    case: tuple[str, str, str, int, str],
) -> dict:
    name, mode, verification, block_size, policy = case
    command = [
        str(runner),
        "--model",
        str(artifact),
    ...
    if mode in ("aurora-replay", "aurora-persistent"):
        command.extend(
    ...
    if result.returncode != 0:
        raise RuntimeError(
            result.stderr.strip() or f"{name} diagnostic failed"
        )
...
def run_ablation(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="k3x-b0018-") as temporary:
        root = Path(temporary)
        config = SyntheticK3Config.default().replace(
            num_experts=24, top_k=16
        )
        ...
        pair_names = {
            name: pair
            for pair, replay_name, persistent_name in PAIRS
            for name in (replay_name, persistent_name)
        }
        records: list[dict] = []
        for case in CASES:
            name, mode, verification, block_size, policy = case
            ...
            if not token_parity or not route_parity or state_error > 1.0e-6:
                raise RuntimeError(
                    f"{name} diverged from natural target execution"
                )
            ...
            if mode == "aurora-persistent" and (
                benchmark.draft_replayed_context_tokens != 0
                or benchmark.draft_context_prefill_tokens != 5
                or benchmark.draft_incremental_forward_calls <= 0
            ):
                raise RuntimeError(f"{name} lacks persistent cursor evidence")
    ...
    records[0]["paired_decode_delta_percent"] = None
    records[0]["draft_reader_byte_reduction_percent"] = None
    for pair_name, replay_name, persistent_name in PAIRS:
        replay = by_name[replay_name]
        persistent = by_name[persistent_name]
        for field in (
            "token_ids",
            ...
            if replay[field] != persistent[field]:
                raise RuntimeError(f"{pair_name} {field} diverged")
        replay["paired_decode_delta_percent"] = 0.0
        replay["draft_reader_byte_reduction_percent"] = 0.0
        persistent["paired_decode_delta_percent"] = (
            persistent["decode_tokens_per_second"]
            / replay["decode_tokens_per_second"]
            - 1.0
        ) * 100.0
        persistent["draft_reader_byte_reduction_percent"] = (
            1.0
