...
Milestone 21 exact resident MoE-layer CUDA execution is partially implemented on branch `codex/milestone-twenty-one-resident-moe-layer`; the portable whole-layer contract, exact CPU oracle, and three low-level CUDA primitives are complete, while the resident backend, runtime dispatch, and B-0022 remain pending. The selected boundary keeps routing on the CPU but joins routed-down, resident expert grid, ordered contribution mixing, RMSNorm, routed-up, shared SiTU MLP, and final addition on one CUDA stream. Milestone 20 remains the latest fully implemented runtime and is public through PR #29 at integration head `90b20c87`.
...
- Milestone 21 runtime/backend inspection, official CUDA Graph constraint review, alternative comparison, accepted design, D-046, detailed TDD plan, checklist, portable API, exact CPU oracle, and ordered-mix/RMSNorm/add CUDA primitives are complete. The resident backend, runtime integration, B-0022, full verification, and publication remain.
- `moe-layer` now has a tested portable contract, CPU oracle, and low-level CUDA primitives, but no complete resident CUDA backend or executable runtime switch yet. Its accepted CUDA contract remains exact FP32/native-MXFP4, resident-only, all-or-nothing before launch, and non-default. CUDA Graph caching and a complete device-resident token graph remain deferred alternatives.
