...
Milestone 17 persistent AURORA draft state is in implementation. The cursor and provider now perform one-time context prefill, exact fixed-K4 proposals, bounded KDA checkpoint restore, MLA logical crop, and target-bonus teacher forcing. Provider-level and token-major/CPU expert-major target parity match the replay oracle on the Top-16 synthetic artifact. CLI/schema integration and B-0018 remain pending. Milestone 16 replay remains the exact non-default oracle and the natural strict target verifier remains authoritative.
