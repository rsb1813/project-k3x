// K3X root 검증을 위한 portable SHA-256을 구현합니다.
...
std::uint32_t load_be(const std::byte* source) {
...
Sha256Hasher::Sha256Hasher() : state_{
        0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,
        0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19,
    } {}

void Sha256Hasher::compress(const std::byte* block) {
    std::array<std::uint32_t, 64> words{};
    for (std::size_t index = 0; index < 16; ++index) words[index] = load_be(block + index * 4);
...
void Sha256Hasher::update(std::span<const std::byte> data) {
    if (finished_) throw std::logic_error("SHA-256 hasher already finished");
    total_bytes_ += data.size();
    while (!data.empty()) {
        const auto count = std::min(buffer_.size() - buffered_, data.size());
        std::copy_n(data.begin(), count, buffer_.begin() + buffered_);
        buffered_ += count;
...
std::array<std::byte, 32> Sha256Hasher::finish() {
    if (finished_) throw std::logic_error("SHA-256 hasher already finished");
    finished_ = true;
    ...
    for (int index = 0; index < 8; ++index) {
        buffer_[56 + index] = static_cast<std::byte>(bit_length >> (56 - 8 * index));
    }
    compress(buffer_.data());
    std::array<std::byte, 32> output{};
    for (std::size_t index = 0; index < state_.size(); ++index) {
        for (int byte = 0; byte < 4; ++byte) {
            output[index * 4 + byte] = static_cast<std::byte>(state_[index] >> (24 - 8 * byte));
        }
    }
    return output;
}
