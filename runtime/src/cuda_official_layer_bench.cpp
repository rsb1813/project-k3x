...
bool validate_portable_oracle(const k3x::bench::LoadedOfficialLayer& loaded,
    ...
    const auto weights = loaded.portable_views();
...
int main(int argc, char** argv) {
    ...
    auto loaded = k3x::bench::load_official_layer_fixture(
        reader.value(), 1, manifest->selected);
    if (!loaded) {
        write_error(k3x::ErrorCode::invalid_extent,
