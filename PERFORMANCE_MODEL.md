...
The accepted next experiment is bounded exact draft-weight residency with the same FP32 routing and target verifier. Reduced precision is deferred so residency benefit and quality divergence are not conflated. A production performance model still needs the resident hit distribution, eviction pressure, expert union size across speculative blocks, target acceptance, native-Linux PCIe throughput, and full-layer GPU occupancy.
