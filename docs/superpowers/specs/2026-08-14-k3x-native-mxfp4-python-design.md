...
Build a native `sm_120` PyTorch CUDA extension whose arithmetic and nibble/scale ordering match `runtime/cuda/mxfp4.cu`. The extension accepts one FP32/BF16 activation vector plus CUDA uint8 packed/scales tensors and returns BF16 output. The current portable `K3XTensorStore.mxfp4_matvec` remains the CPU/reference fallback.
