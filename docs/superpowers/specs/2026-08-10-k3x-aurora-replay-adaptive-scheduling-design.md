...
The scheduler selects the longest eligible rung within the request maximum. Any block with `accepted_draft_tokens < proposed_draft_tokens` immediately caps the next choice at the preceding rung. This rejection backoff takes precedence over exploration. Requests with maximum zero return zero candidates without mutating scheduler state.
