...
Local commits through `3dd0a80` add `--scope two-layer` dry-run/materialization, a deterministic two-layer oracle, execution-ordered K3X v1 assembly, strict two-layer metadata, crash-safe resume, Reader verification, and a portable C++20 A1→A2→B1→B2 orchestrator. The orchestrator owns two independent KDA states, feeds each layer-1 output directly into layer 2, and delegates every Attention Residual, KDA, routing, and MoE operation to the already parity-tested exact layer primitive. Four focused official CTests, two focused cross-language tests, strict warning compilation of the new sources, and four ASan/UBSan tests pass. This remains synthetic and mocked-transport correctness evidence only: no official payload was downloaded. The CUDA two-slot state registry, activation bridge, B-0034 measurement, full checkpoint, token throughput, and paid Cloud Run work do not exist yet.
