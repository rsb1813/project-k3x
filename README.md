...
M29 is in progress; its metadata planner, independent PyTorch and portable C++ KDA oracles, bounded complete-layer manufacturing path, tiny portable complete-layer composition, strict pinned preflight, and official-weight portable execution gate are implemented. The native CUDA complete-layer boundary and B-0030 remain unimplemented. It extends the bounded official fixture from the measured MoE FFN boundary to the complete layer-1 graph: self Attention Residual, input RMSNorm, KDA with incremental convolution/recurrent state, MLP Attention Residual, natural Top-16 Stable LatentMoE, and final prefix accumulation.
