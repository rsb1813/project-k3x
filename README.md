...
The checkpoint stores channel-wise F32 `A_log[128]`, while its pinned Python constructor creates `[96]`. K3X treats the checkpoint header and KDA paper as authoritative, fails closed on `[96]`, and validates a two-token full call against incremental A-then-B execution including all convolution state, FP32 recurrent state, routes, contributions, and layer outputs. The artifact remains non-executable through `k3x_run`; the benchmark-only CUDA harness does not weaken that production guard. This milestone makes no token-rate or quality claim.
...
Task 3 reuses the M28 content-addressed range objects, verified partial resume, source assembler, expert planner, and non-executable optional-feature guard. It materializes the 17 KDA objects and 11 MoE trunk objects before deriving two full/incremental state-linked natural routes, atomically publishes `route-state-manifest.json` before fetching only the selected expert union, and packs tensors in exact layer execution order. The live `--scope kda-layer --dry-run` matched the pinned revision and source blob with zero tensor-payload bytes. The later bounded run selected experts `[348,65,544,421,379,753,839,423,169,60,818,629,375,225,200,178]` and `[824,11,302,546,16,211,176,192,501,692,511,887,442,558,810,556]`. Converter/recovery coverage passes 123 tests; no complete shard, checkpoint, or paid resource was used.

Task 4 adds a Reader-free portable C++ KDA boundary over native BF16 words and F32 convolution/decay/state data. It validates checked dimensions, shapes, finite values, and V-first state before computation; exposes every projection, convolution, gate, recurrence, and output boundary; and publishes no partial result on failure. Full and incremental execution match at every boundary, and the complete dump matches the independent PyTorch oracle within `1e-6`. CPU CTest passes 18/18 and Python C++ parity passes 114 tests with 32 capability skips. This remains a tiny correctness oracle, not official execution or performance evidence.

Task 5 composition now executes the exact tiny complete-layer order with the existing portable KDA and MoE authorities. The owned result exposes both Attention Residual outputs, input normalization, KDA boundaries/state, post-KDA prefix, normalized MoE input, natural route/contributions, every MoE boundary, and final prefix output. Full and incremental runs match for every step and state, while an independent PyTorch graph reproduces both changing routes and outputs within `1e-6`. CPU CTest passes 19/19 and Python C++ parity passes 115 tests with 32 capability skips.

Task 5 preflight parses duplicate-key/non-finite-safe JSON and validates the pinned repository/source/header identity, V-first state chain, fixed input hashes, natural route union, exact 17 KDA and 11 MoE source ranges, artifact root, converter source fingerprint, tensor execution order, dtype, shape, and every data/auxiliary SHA-256 before any CUDA backend can be constructed. It also reconstructs the deterministic safetensors header and payload stream to verify the separate microshard SHA-256. The executable remains deliberately fail-closed with `BACKEND_UNAVAILABLE` after successful preflight until the official CPU/CUDA execution path is implemented. Focused preflight/Reader/converter coverage passes 36 tests with 4 capability skips, the wider converter/source-integrity regression passes 123 tests, and full CPU/CUDA CTest pass 19/19 and 32/32. This is correctness evidence only, not official payload execution or B-0030.

Task 5C loads all official BF16/F32 KDA and MoE trunk weights plus the exact 32-expert MXFP4 union from the checksum-verified Reader and executes the portable complete-layer oracle both full and incrementally. The materializer now atomically writes a 6,541,344-byte source-byte PyTorch oracle sidecar and binds its SHA-256 into the route manifest. PyTorch and portable scalar reductions intentionally use different accumulation orders, so correctness uses measured absolute gates while retaining exact full/incremental parity and exact route IDs. Maximum observed differences are `1.52588e-05` for KDA output, `0.0078125` for Q/K convolution state, `0.00390625` for V convolution state, `4.39133e-05` for recurrent state, and `1.06171e-06` for route contributions. The official artifact reaches the intended `BACKEND_UNAVAILABLE` only after all these gates pass. These are numerical correctness observations, not latency or token-rate measurements.
...
Local commits through `af762e6` add `--scope two-layer` dry-run/materialization, a deterministic two-layer oracle, execution-ordered K3X v1 assembly, strict two-layer metadata, crash-safe resume, Reader verification, a portable C++20 A1→A2→B1→B2 orchestrator, a capacity-two CUDA KDA state registry, and an opaque inter-layer activation bridge. The front accepts host input only at layer 1 or consumes a single-use preceding-layer token, retains hidden plus the unchanged block source in two grow-only ping-pong slots, and returns raw router logits plus opaque prepared/state tokens. Canonical natural routing remains on the host. The tail reuses the exact prepared MXFP4/shared FFN and either retains the result for the next layer or publishes only the requested final hidden vector. Wrong owner, producer, consumer, width, slot, stale token, double consumption, and failed-tail cleanup fail closed. CUDA MoE/KDA/layer CTest 3/3, non-CUDA unavailable CTest, two historical device-state/route Python regressions, strict CUDA warning compilation, and Compute Sanitizer on both official MoE and layer targets with zero errors pass. The retained layer-1-to-layer-2 path adds no hidden-vector D2H/H2D; these are logical runtime counters, not measured physical PCIe traffic. This remains bounded correctness evidence only: no official M33 payload was downloaded. The dedicated two-layer wrapper, closed telemetry, B-0034 measurement, full checkpoint, token throughput, and paid Cloud Run work do not exist yet.
