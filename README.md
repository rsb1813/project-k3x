...
- [x] Authenticated local manufacture is active with native MXFP4 experts and group-128 8-bit selected trunk matrices; 14 verified fragments are complete and every source file was ledger-gated before deletion.
...
- [x] Add two bounded RAM-staging Foundry workers, combined output audits, immutable source-deletion identities, resume-byte accounting, and staging-before-prefetch scheduling.
...
- The quality Local Foundry has produced 14 Reader-valid official fragments. Two bounded RAM conductors currently cover shards 40–66 and 71–96; shards 8–36 follow when a RAM slot becomes available. Cross-worker HDD-to-RAM staging is serialized while conversion and prefetch remain concurrent. The checksum-bound fragment-set format, Reader, and compatibility graph binding are implemented, but the official set and first token do not exist yet. Provenance is pinned official shard SHA-256 plus K3X roots and the local ledger, not signed publisher provenance.
