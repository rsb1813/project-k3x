...
Local commits through `9265799` add `--scope two-layer` dry-run/materialization, a deterministic two-layer oracle, execution-ordered K3X v1 assembly, strict two-layer metadata, crash-safe resume, Reader verification, a portable C++20 A1→A2→B1→B2 orchestrator, a capacity-two CUDA KDA state registry, an opaque inter-layer activation bridge, and an exact two-mode CUDA wrapper. The wrapper runs the same interleaved trace as either `host_round_trip` or `device_closure`, keeps independent device KDA state for layers 1 and 2, publishes both final states, retains canonical host routing, and reports bounded logical traffic categories without token/TPS fields. Tiny CUDA tests match the portable oracle for all four routes, contributions, both outputs, and both final states; missing experts, swapped layers, invalid modes, insufficient residency, and post-failure backend reuse fail closed. CUDA MoE/KDA/layer/two-layer CTest 4/4, strict compilation of the changed C++/CUDA sources, and Compute Sanitizer on the new wrapper with zero errors pass. The device-closure row reports zero logical inter-layer hidden D2H/H2D; this is not measured physical PCIe traffic. This remains bounded synthetic correctness evidence only: no official M33 payload was downloaded. The dedicated artifact/manifest/oracle harness, B-0034 measurement, full checkpoint, token throughput, and paid Cloud Run work do not exist yet.
