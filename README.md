...
Local commits through `8580df8` add `--scope two-layer` dry-run/materialization, a deterministic two-layer oracle, execution-ordered K3X v1 assembly, strict two-layer metadata, crash-safe resume, Reader verification, a portable C++20 A1→A2→B1→B2 orchestrator, and a capacity-two CUDA KDA state registry. The portable orchestrator delegates numerical work to the already parity-tested exact layer primitive. The CUDA registry retains independent layer-1/layer-2 states in fixed grow-only buffers, uses backend-wide owner and unique generation tokens, preserves state-transfer accounting, rejects live overwrite and a third layer, and prevents a host round trip for one layer from invalidating the other. CUDA MoE/KDA/layer CTest 3/3, non-CUDA unavailable CTest, 21 M31 device-state regressions, strict CUDA warning compilation, and Compute Sanitizer with zero errors pass. This remains bounded correctness evidence only: no official M33 payload was downloaded. The activation bridge, B-0034 measurement, full checkpoint, token throughput, and paid Cloud Run work do not exist yet.
