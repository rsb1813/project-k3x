...
- Date: 2026-08-11.
- Status: accepted and measured as an explicit experiment; host route preparation remains the default.
...
- Benchmark result: B-0033 host/device medians are 64.210407/63.767134 ms, a -0.690344% change. Device kernel time rises 27.294141%, orchestration falls 9.279958 ms per sequence, resident weights rise 12,888,064 bytes, and logical D2H rises 7,168 bytes per sequence. Exact routes, output/state digests, zero warm weight H2D, and `0.00048828125` maximum error are preserved.
...
- Evidence: the released graph identifies layers 1 and 2 as KDA plus MoE. M33 implements dependency-ordered bounded manufacture, exact A1→A2→B1→B2 portable execution, a capacity-two state registry, the opaque hidden/block bridge, both CUDA modes, the dedicated trust-boundary harness, and the strict two-row B-0034 evidence transaction. The 3,641,057,536-byte artifact contains 119 verified range objects from two pinned shards. Both measured rows preserve exact expert IDs, pass contribution/output/state semantic gates, and transfer zero warm weight bytes.
- Benchmark result: B-0034 host-round-trip/device-closure medians are 96,102,951/109,388,034 ns. Device closure removes 57,344 logical inter-layer H2D and D2H bytes per sequence but is 13,285,083 ns or 13.823803% slower, adds 86,016 resident bytes, and raises tracked peak device memory by 258,048 bytes. Host round trip remains the default.
...
- Date: 2026-08-13.
- Status: implemented, concurrency-tested, and active across three disjoint workers; aggregate timing is pending.
...
- Benchmark result: focused ledger resume, two-process concurrent publication, and source-deletion gates pass 3/3, including a two-process check on the actual C-drive mount. The parameterized PowerShell conductor parses successfully. Workers are concurrently converting shards 7, 37, and 67; no multi-worker throughput result is claimed before completed publications.
