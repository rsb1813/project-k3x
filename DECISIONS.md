...
- Date: 2026-08-12.
- Status: accepted and implemented locally; actual-artifact measurement pending.
...
- Evidence: the released graph identifies layers 1 and 2 as KDA plus MoE. Local M33 now implements dependency-ordered two-layer manufacture, exact A1→A2→B1→B2 portable execution, a capacity-two state registry, the opaque hidden/block bridge, both CUDA modes, the dedicated trust-boundary harness, and the strict two-row B-0034 evidence transaction. The evidence and historical B-0030 through B-0033 suites pass 87/87, CUDA two-layer CTest passes, and no official M33 payload has yet been executed. B-0033 shows four added kernels and two logit synchronizations offset most of the removed host route work, so another isolated one-layer micro-optimization cannot test activation-residency amortization.
...
- Rejected claims: the local implementation does not measure B-0034, prove arbitrary-layer or concurrent-session state management, remove all router-logit synchronizations, authorize a default change, establish token throughput or quality, download a complete checkpoint, or provision paid cloud resources.
