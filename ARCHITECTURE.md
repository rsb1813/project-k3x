...
Task 1 implements and tests only the bounded metadata planner extension: official MoE and KDA plans accept exact layer IDs 1 and 2, preserve canonical layer-specific names/shard bindings/byte contracts, and reject IDs outside that pair. Existing layer-1 materialization remains fixed and unchanged. The dependency-ordered two-layer oracle, artifact, state registry, activation bridge, harness, and B-0034 remain proposed.
