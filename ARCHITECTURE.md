...
## Milestone 31 partial official KDA device-state boundary

M31 Task 1 is implemented and tested at the raw official KDA backend boundary. The three convolution histories and V-first recurrent state now use one dedicated CUDA-backend allocation, separate from sequence-sized operation scratch. An opaque backend-owner/generation token expresses state seed, single-use continuation, and explicit final publication without exposing a device pointer. Host round trip remains the source-compatible default and correctness oracle.

Only one device state is implemented per backend for this experiment. Tokens bind layer and KDA configuration, reject stale or cross-backend use before transfer or launch, and become invalid once a state-mutating operation begins. The official-layer wrapper propagates seed/final-publication controls, automatically discards a token after downstream routing or MoE failure, and preserves the source-compatible host default. The harness exposes `--state-transfer host|device` only as an explicit closed-schema option and restricts device state to incremental resident admission. Tiny and actual-fixture correctness smokes pass with half the logical state traffic. The strict B-0032 transaction and verifier are implemented for fixed host-incremental/device-incremental/full-host rows, but no formal B-0032 result exists. Multi-session residency, concurrency policy, eviction, and VAULT integration remain unimplemented.
