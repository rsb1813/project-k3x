...
The Python first-token compatibility path now has a name-to-ID fragment tensor store. It decodes native BF16/F32 and group-128 Q8 dense records, reads bounded BF16 row slices for embeddings and the LM head, and performs literal native-MXFP4 expert matvecs from K3X extents. The layer-0, KDA-MoE, MLA-MoE, resumed-layer, and LM-head entrypoints accept only a sealed `K3XSET1` for local payload execution; the Python parser verifies the manifest digest, fragment metadata, and internal roots before strict Reader opening. This is an executable graph-binding compatibility path, not the optimized final CUDA runtime; dense Q8 expansion, literal MXFP4 materialization, repeated process startup, and strict-open traffic remain profiling targets.
