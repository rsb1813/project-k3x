...
The later D-076 user instruction superseded that download lock with a different quality recipe rather than accepting the 3-bit candidate. The completed Local Foundry preserves native MXFP4 expert packed codes and scales, uses group-128 signed 8-bit codes with BF16 scales only for selected two-dimensional BF16 trunk matrices, and passes sensitive tensors through. Python and C++ readers negotiate required feature bit 2 for this representation. All 96 official source shards became independently Reader-valid K3X fragments totaling 1,507,512,467,456 payload bytes. The published `K3XSET1` manifest seals the plan, ordered fragment filenames, lengths, internal roots, and canonical record SHA-256 `f5c7443fd9ea9b4a2f0c95010f148182eefaedec4f29c094ca24e6bc4e61cefe`; publication required no second checkpoint-sized payload copy.
...
The Python first-token compatibility path has a name-to-ID fragment tensor store. It decodes native BF16/F32 and group-128 Q8 dense records, reads bounded BF16 row slices for embeddings and the LM head, and performs literal native-MXFP4 expert matvecs from K3X extents. The layer-0, KDA-MoE, MLA-MoE, resumed-layer, and LM-head entrypoints accept only a sealed `K3XSET1` for local payload execution; the Python parser verifies the manifest digest, fragment metadata, and internal roots before the D-081 directory-open policy. The full path executed all 93 layers and emitted the same greedy token as the independent original-precision oracle. This remains graph-binding compatibility infrastructure, not the optimized final CUDA runtime.

D-098 adds an explicit persistent HTTP metadata cache to the official compatibility entrypoints. Its key binds the URL and normalized request headers; every cached body is paired with an atomic metadata record and SHA-256, and a digest mismatch forces a network refetch. This removes repeated snapshot/index/config/header transfer without weakening the pinned topology, index, config, header, set, or layer-state checks. It does not cache model payloads and is not a replacement for the future persistent C++ token process.

D-081 separates manufacture-time payload proof from repeated execution-time directory opening. Strict Reader remains the default and conversion/source deletion still requires full extent CRC and root validation. A compatibility runner that has verified the sealed set reads and validates the superblock, directories, feature contract, extent layout, and directory digest but trusts the already recorded payload root instead of rescanning the complete fragment on every layer subprocess.

D-084 makes K3X graph provenance transitive. Layer 0 writes the exact set-manifest digest into prefix state; every KDA/MLA layer verifies it before compute and republishes it; the LM head checks the same identity. Every K3X layer and head result also names the set digest and storage source, so zero network traffic is not used as a proxy for artifact identity.
...
D-085 optionally places a worker's bounded conversion directory on WSL tmpfs. When the source and work roots are on different filesystems, the converter copies the authenticated shard once into the temporary root, verifies SHA-256 and inspects tensors from that copy, then performs Q8 derivation and K3X packing from RAM. The original D-drive source remains the durable deletion authority. WSL is now capped at 72 GiB with 16 GiB swap, and `/dev/shm` is explicitly mounted at 60 GiB so two approximately 17 GiB work roots can coexist with bounded derived data.
...
## Milestone 39 persistent official compatibility runtime

The official Python compatibility stages now expose callable layer-0, KDA-MoE, MLA-MoE, and LM-head entrypoints. The persistent token driver owns one process across the complete graph, resumes from the authenticated prefix-state manifest, validates each publication, and retains the subprocess chain as an explicit reference mode.
...
## Milestone 44 native MXFP4 expert-major residency

The official Python compatibility runtime now has an experimental native `sm_120` MXFP4 path that keeps E2M1 codes and E8M0 scales packed through device admission. `OfficialRuntimeContext` owns explicit byte-bounded host and CUDA caches; zero-byte defaults preserve the portable reference path. The first implementation caches individual gate, up, and down matrices rather than claiming atomic whole-expert residency.

The accepted CUDA batch boundary executes one natural routed set as an expert-major grid. It evaluates all gate/up projections, applies SiTU, evaluates the down projections, and accumulates contributions in deterministic route order. B-0060 proves BF16 bit-exact output against the scalar direct path for the official layer-1 Top-16 route. Two scalar micro-optimizations were measured and reverted because B-0057 and B-0059 were slower than B-0056.
