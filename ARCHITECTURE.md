...
The dedicated `k3x_cuda_official_two_layer_bench` is now implemented as a benchmark-only trust boundary. It parses duplicate-key-safe JSON, binds the fixed official revision/source/shards, validates exact layer and step order, route unions, state-chain digests, oracle header/hash, K3X root, record layer identity, execution-order extents, and every dense or MXFP4 packed/scale digest before backend construction. A shared parameterized loader owns the released layer-1/2 dtype and shape contract and is also used by the historical one-layer harness. The harness recomputes the complete portable two-layer graph and all step route/state/KDA/output digests before executing either CUDA mode. Measured iterations require zero warm weight H2D when warmup is nonzero and emit bounded telemetry with no token/TPS fields. No dynamic map, eviction, session registry, arbitrary-layer, concurrency, or production graph claim is introduced. Actual-artifact execution and B-0034 remain pending.

The B-0034 evidence boundary is implemented separately from the CUDA harness. It admits only the fixed host-round-trip/device-closure order and revalidates the pinned revision, snapshot, index, config, source blob, two shards, A1→A2→B1→B2 dependency chains, per-layer expert unions, oracle identity, K3X root/source/tensor digests, and all transaction input hashes. The two records must retain route, contribution, output, and final-state identities; warm weight H2D must be zero; logical transfer decomposition and state/prepared lifetimes must close; and the resident byte formula must match the selected union. Canonical raw JSON, LF CSV, aggregate hashes, fsync, and atomic publication form one fail-closed evidence transaction. The verifier deliberately performs no ranking or timing interpretation, and the production runtime remains unchanged.

K3X v1 already supports canonical tensor names and per-layer directories, so Task 2 uses one execution-ordered two-layer bounded artifact rather than a format revision. The synthetic/mocked-transport manufacturer passes 139 focused and historical converter tests at local commit `324822f`. The portable runtime at `3dd0a80`, two-slot registry at `8580df8`, activation bridge at `af762e6`, and wrapper at `9265799` retain their recorded gates. The harness at `d13f88e` passes 36 historical/new one-layer and two-layer Python tests, CUDA official MoE/KDA/layer/two-layer CTest 4/4, portable two-layer CTest, strict changed-source compilation, the production guard, and Compute Sanitizer with zero errors. These are synthetic correctness and negative trust-boundary results, not an actual M33 artifact execution, performance measurement, or physical PCIe measurement. No B-0034 result, full-checkpoint execution, token throughput, quality result, or default change exists yet. The complete design is in [`docs/superpowers/specs/2026-08-12-k3x-official-two-layer-device-closure-design.md`](docs/superpowers/specs/2026-08-12-k3x-official-two-layer-device-closure-design.md).
