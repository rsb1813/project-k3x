...
The runtime validates proposal bounds and anchor identity before target mutation, applies only accepted history to one-token target forwards, and returns final recurrent state plus complete routing/K diagnostics when requested. Perfect and mixed mismatch/empty integration traces match greedy tokens, final state, routing, logical Reader traffic, and L1 counters. CPU CTest passes 12/12 at development head `027b65c`. CLI exposure and B-0014 remain in progress, so no speculative throughput or acceptance-rate measurement exists.
