...
Task 1 implements and tests the bounded metadata planner extension: official MoE and KDA plans accept exact layer IDs 1 and 2, preserve canonical layer-specific names/shard bindings/byte contracts, and reject IDs outside that pair. Task 2 now composes those plans only when their pinned source/index and nested MoE identities agree, and implements the pure A1→A2→B1→B2 scheduler with independent layer states, unchanged block propagation, inter-layer activation digests, and per-layer first-use expert unions. Existing layer-1 materialization remains fixed and unchanged. The source-byte executor, dependency-ordered materializer/artifact, state registry, activation bridge, harness, and B-0034 remain proposed.

K3X v1 already supports canonical tensor names and per-layer directories, so the design uses one execution-ordered two-layer bounded artifact rather than a format revision. Future manufacturing remains range-bounded, checksum-bound, dependency-ordered, crash safe, and resumable across each layer's potentially distinct source shard. No two-layer artifact, B-0034 result, full-checkpoint execution, token throughput, quality result, physical traffic result, or default change exists yet. The complete design is in [`docs/superpowers/specs/2026-08-12-k3x-official-two-layer-device-closure-design.md`](docs/superpowers/specs/2026-08-12-k3x-official-two-layer-device-closure-design.md).
